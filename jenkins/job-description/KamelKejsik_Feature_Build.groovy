...
def job_config = [
    job: [
        name: "KamelKejsik_Feature_Build",
        agent: "maven"
    ]
]
def config = [
...
def customLib = library identifier: 'KamelKejsik_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/KamelKejsik_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.KamelKejsik.new()
    ...
    stage('Prepare') {
      steps {
        ...
        script {
          ...
          jslGit.checkout( config, "jarro06", "KamelKejsik", branch_name)
          }
        }
      }
stage('Build') {
...
"""
def job = pipelineJob("KamelKejsik_Feature_Build");
...
job.with {
    ...
    authenticationToken('xaHZuaWrYGxjDz3WOh06YV4w2sx9Zt6L')
